...
if __name__ == '__main__': 
    ...
    for i in range(0, len(data)):
        for n in range(1, N+1):
            ...
            F = data[i].F_max*(data[i].Fbang_pos_R[n].value - data[i].Fbang_neg_R[n].value) #- data[i].dq0[n,'r_R'].value*data[i].damping - data[i].FhardStop_ext_R[n].value + data[i].FhardStop_rtn_R[n].value
            # ==if(F>=data[0].F_max or F<=-data[0].F_max):
            pub[2].publish(F)
            F = data[i].F_max*(data[i].Fbang_pos_L[n].value - data[i].Fbang_neg_L[n].value) #- data[i].dq0[n,'r_L'].value*data[i].damping - data[i].FhardStop_ext_L[n].value + data[i].FhardStop_rtn_L[n].value
            # if(F>=data[0].F_max or F<=-data[0].F_max):
            pub[3].publish(F)
